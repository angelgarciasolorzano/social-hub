...
- **Auth** (`app/Auth/`) — login, register, email verification, password reset (via Fortify), 2FA + trusted devices.
  ...
  - **Trusted device reactivation + GDPR purge (SOC-21)** — `TrustedDevice` uses Laravel's `SoftDeletes` trait (column `deleted_at`) so revoked devices stay in the table for re-confirmation. The `reactivate` endpoint restores a soft-deleted device silently when the browser still holds the matching `trusted_device` cookie, and requires a 2FA OTP (`TrustedDeviceReactivateRequest` with conditional `requiredIf(closure)` + `withValidator.after` mirroring Fortify's TOTP/recovery check) when the cookie is gone. The `force-destroy` endpoint (`DELETE /user/trusted-devices/{device}/force`, requires password confirmation via `TrustedDeviceDestroyForceRequest`) records a final `Revoked` event for snapshot continuity then `forceDelete()`s the row. The `TrustedDevicePurge` Artisan command (`trusted-devices:purge`) is registered in `AuthServiceProvider` (only when `runningInConsole()`) and scheduled daily at 03:00 — it `forceDelete()`s soft-deleted rows older than `module.auth.trusted_devices.purge_after_days` (default 90).
